...
export const Route = createFileRoute("/_layout/dash/")({
  component: Dashboard,
});
...
function Dashboard() {
  ...
  const { copiedId: copiedKeyId, copy: copyKey } = useCopyToClipboard();
  const { copiedId: copiedCmdId, copy: copyCmd } = useCopyToClipboard();
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        toast.success(`Key "${result.name}" created`);
      } catch (error) {
  ...
  return (
    <>
      <div className="inner min-h-lvh relative py-40 px-8 flex h-full flex-col gap-10 border-x">
        <div className="flex flex-col gap-6">
          ...
          <p className="text-muted">
            API keys authenticate your CLI deploys. Because Better Sol is still
            experimental, compiler usage is rate limited. Use an API key to get
            100 compiles per hour instead of 20.
          </p>
            ...
            <Table.ScrollContainer>
              <Table.Content aria-label="API keys">
                <Table.Header>
                  ...
                  <Table.Column>Login Command</Table.Column>
                    ...
                    <Table.Row id={key.id}>
                      ...
                      <Table.Cell>
                        <InputGroup variant="secondary">
                          <InputGroup.Input
                            disabled
                            value={`npx @better-sol/cli@alpha login ${key.keyPrefix}****`}
                          />
                          <InputGroup.Suffix className="pr-0">
                            <Button
                              isIconOnly
                              ...
                              onPress={async () => {
                                try {
                                  const { key: rawKey } = await revealApiKeyFn({
                                    data: { id: key.id },
                                  });
                                  if (!rawKey) {
                                    toast.danger("Key not found");
                                  ...
                                  const cmd = `npx @better-sol/cli@alpha login ${rawKey}`;
                                  copyCmd(key.id, cmd);
                                  toast.success(
                                    "Login command copied to clipboard",
                                  );
                                } catch {
                                  toast.danger("Failed to copy login command");
                                }
                              }}
                            >
                              {copiedCmdId === key.id ? (
