      ...
      return Promise.all(
        transactions.map(async (tx) => {
          ...
          try {
            ...
            signed = (await provider.signTransaction!(vTx)) as { readonly signatures: readonly (Uint8Array | null)[] };
          } catch {
            const legacyTx = web3.Transaction.from(wireBytes);
            signed = (await provider.signTransaction!(legacyTx)) as { signature: Uint8Array | null };
          }

          if (signed === null) {
            throw new Error("Reown signTransaction returned null");
          }
