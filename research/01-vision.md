...
packages/better-sol/          Runtime library (ships to browsers)
  ...
  src/client/
    factory.ts                betterSol() entry, Proxy-based program client
    transaction.ts            Build/sign/send with WebSocket confirmation
    events.ts                 Typed error parsing + Anchor event decoding
    bound-account.ts          PDA derivation + account fetch
    token-client.ts           Token + Token-2022 operations
    lookup-tables.ts          Address lookup table resolution
    signer.ts                 Keypair loading, seed encoding
    types.ts                  All type definitions, config types
  src/wallets/
    wallet-adapter.ts         @solana/wallet-adapter integration + re-exports
    reown.ts                  Reown AppKit adapter
    privy.ts                  Privy adapter
    dynamic.ts                Dynamic adapter
    sign-utils.ts             Shared signing utilities

packages/cli/                 CLI tool (never shipped to browsers)
  src/parser/                 ts-morph AST parser + file discovery
  src/generator/              Rust, IDL, Drizzle generators
...
apps/web/                     better-sol.fun website + playground
```
...
The transpiler rejects 18 categories of unsupported TypeScript patterns with specific diagnostic messages naming the exact line and suggesting a fix. It does not silently generate broken Rust.
...
| Proxy-based program client | tRPC `createRecursiveProxy` pattern | Eliminates `Record<string, unknown>` intermediate |
| WebSocket-first confirmation | `signatureNotifications` subscription over polling | ~400ms confirmation latency vs 1-5s polling |
| Instance methods for error/event parsing | `sol.counter.parseErrors(logs)`, not standalone | Discoverability, caching, encapsulation |
| `#*` import aliases | Node.js subpath imports with wildcard | Zero-maintenance internal imports, extensible |
| tsdown as sole bundler | Not tsc + bun build | Single tool, DTS generation, platform-aware |
