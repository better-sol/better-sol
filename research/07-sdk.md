...
> **Seed semantics:** Account seeds are defined only through `.derive((seed) => ['literal', seed.fieldName])`. Dynamic seed values must be stored as pubkey/integer fields on the account. During `p.create(Account)` generation, each seed field must be supplied by an instruction arg or account with the same name; raw `'{argName}'` string templates are rejected.
...
Callers who already have a Kit-compatible `TransactionSigner` pass it directly to `sol.withSigner(myKitSigner)`.
...
| Direct Phantom | `window.phantom.solana.signTransaction()` | `walletAdapter()` |
